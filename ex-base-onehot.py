...
for SEED in tqdm(SEEDS):
    ...
    CRITICAL    = nn.CrossEntropyLoss().cuda()
    ...
    for EPOCH in range(1, EPOCHS + 1):
        ...
        OPTIMIZER.step()
        if ONE_HOT_ENCODING:
            ACCURACY = (PREDICT.argmax(dim=1).float() == LABEL.argmax(dim=1).float()).float().mean()
        else:
